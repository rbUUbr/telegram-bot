...
Telegram::Bot::Client.run(TOKEN) do |bot|
  bot.listen do |message|
    ...
    when '/status'
     StatusCommand.command(message, bot,"Так-с, что тебе надо сдать...Ах, да, вот что:", user.subjects)
    when '/reset'
      ResetCommand.command(user.subjects, user.start_semester, user.end_semester)
    when '/submit', /я сдал/
      MainCommand.command(message, bot, 'красава')
    else
    	case user.action
    	when 'semester'
        SemesterCommand.semester(message, bot, user, message.from.id)
        #write_semester_to_file(message.from.id, user.start_semester, user.end_semester)
      when 'subject'
        bot.api.send_message(chat_id: message.chat.id, text: "Сколько лаб по предмету?")
        temp = message.text
        user.action = 'count_of_labs'
      when 'count_of_labs'
        SubjectsCommand.end_command(bot, message, message.from.id, temp, message.text, user.subjects)
        bot.api.send_message(chat_id: message.chat.id, text: "Оке")
      end
    end
  end
end
